        ...
        <div class="container">
            <div class="row">
                ...
                <div class="card-02" id="div-rem">
					...
					if(isset($_POST['srremstud'])){
						$rstud=$_POST['rvalue'];
						$sql="SELECT * FROM registration WHERE name like '%$rstud%'";
			...
			<div class="row">
				<div class="card-03">
					...
					<?php
					$max_query = "SELECT COUNT(*) AS max_value FROM registration WHERE sem=$usersem AND type=0";
					$result = $conn->query($max_query);
					$row = $result->fetch_assoc();
					$max_value = $row['max_value'];
					for($i=1; $i<=$max_value; $i++){
						echo "<center>";
						echo "<div class='card-02'>";
						$sql="SELECT * FROM registration where tid='$user'";
						$result=$conn->query($sql);
						if($result->num_rows>0){
							$z=1;
							$query ="SELECT subjects.admn, registration.admn, subjects.grade, subjects.code
									FROM subjects
									JOIN registration
									ON subjects.admn = registration.admn
